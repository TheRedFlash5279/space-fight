...
function moveSpriteRandom (sprite: Sprite, yLowerBound: number, outerBound: number, v: number) {
    moveSprite(sprite, Math.randomRange(outerBound, scene.screenWidth() - outerBound), Math.randomRange(outerBound, yLowerBound), v)
}
function nonSpell1 () {
...
function moveSpriteRandomFixedTime (sprite: Sprite, yLowerBound: number, outerBound: number, t: number) {
    moveSpriteInTime(sprite, Math.randomRange(outerBound, scene.screenWidth() - outerBound), Math.randomRange(outerBound, yLowerBound), t)
}
...
function enemyShootAimingPlayer (sprite: Sprite, speed: number, spread: number) {
    shootBulletFromSprite(sprite, speed, Math.atan2(mySprite.y - sprite.y, mySprite.x - sprite.x) * 57.3 + Math.randomRange(0 - spread, spread))
}
let bossProgress = 0
